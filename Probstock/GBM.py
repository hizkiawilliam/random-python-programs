...
'''
Stock prices
- FSE/EON_X, Frankfurt Stock Exchange/EON
- FSE/ALV_X, Frankfurt Stock Exchange/Allianz
- FSE/ADS_X, Frankfurt Stock Exchange/Adidas

prices = quandl.get("FSE/EON_X", 
                authtoken="C7yzgFz1vFDy9qW_HrqY", 
...
prices = quandl.get("FSE/EON_X",
                authtoken="C7yzgFz1vFDy9qW_HrqY", 
                start_date = '2019-01-01', end_date = '2019-08-31'
                ).reset_index(drop = False)[['Date', 'Close']]
...
price_to_predict = quandl.get("FSE/EON_X", 
               authtoken="C7yzgFz1vFDy9qW_HrqY", 
...
returns = (price_to_predict.loc[1:, 'Close'] - \
           price_to_predict.shift(1).loc[1:, 'Close']) / \
           price_to_predict.shift(1).loc[1:, 'Close']
#print(returns)

# Parameter Assignments
So = price_to_predict.loc[price_to_predict.shape[0] - 1, "Close"]
dt = 1 # day   # User input
...
scen_size = 5 # User input (menentukan seberapa banyak scenarionya
...
print(price_to_predict["Date"].max())
for i in range(scen_size):
    plt.title("Frankfurt Stock Exchange/EON: " + str(sigma))
    #plot semua prediksi sebanyak scenario
    
    plt.plot(pd.date_range(start = price_to_predict["Date"].max(), 
                end = pred_end_date, freq = 'D').map(lambda x:
                x if x.isoweekday() in range(1, 6) else np.nan).dropna(), S[i, :])
    plt.ylabel('Stock Prices, €')
    plt.xlabel('Prediction Days')
plt.plot(prices['Date'], prices['Close']) #plot harga original
plt.show()

# Dataframe format for predictions - first 10 scenarios only

Preds_df = pd.DataFrame(S.swapaxes(0, 1)[:, :10]).set_index(
           pd.date_range(start = price_to_predict["Date"].max(), 
           end = pred_end_date, freq = 'D').map(lambda x:
